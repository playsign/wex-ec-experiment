...
function init() {
	...
	jsonLoader.load(ASSET_PREFIX + "Masterscene.js", function(geometry, material) {
		addModelToScene(geometry, material, "oulu");
	});
	jsonLoader.load(ASSET_PREFIX + "ColliderBuildings.js", function(geometry, material) {
		addModelToScene(geometry, material, "colliderbuildings");
	});
	jsonLoader.load(ASSET_PREFIX + "ColliderGround.js", function(geometry, material) {
	...
	car.loadPartsJSON(ASSET_PREFIX + "GreenCar.js", ASSET_PREFIX + "GreenCar.js");
	...
	if (type == "oulu" && debugMode === false) {
		...
		for (var i = 0; i < materials.length; i++) {
			if (materials[i].map) {
				...
				var texpath = ASSET_PREFIX + "images/" + ddsName;
...
function update() {
	...
	if (flyMode === true) {
		...
		flyControls.update(Date.now() - appStartTime);
	} else {
		if (car && car.bodyMesh) {
...
var componentDispatch = {
        EC_Mesh: function(ent, comp) {
                jsonLoader.load(comp.meshRef.ref, function(geometry, material) {
                        comp.threeGeometry = geometry;
                        addModelToScene(geometry, material, comp.name);
                });
        },
};
function populateThree() {
        // find what models are in the entity system and load them accordingly
        for (var i in Object.keys(scene.entities)) {
                var ent = scene.entityById(i);
